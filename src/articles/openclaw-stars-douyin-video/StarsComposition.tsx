...
const CHART_MARGIN = { top: 150, right: 100, bottom: 150, left: 150 }
...
const StarGrowthChart: React.FC = () => {
  ...
  // 纵向增长：y轴=时间，x轴=stars
  const yScale = CHART_HEIGHT / (dataLength - 1)
  const xScale = CHART_WIDTH / maxStars
    ...
    >
      ...
      >
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((ratio, i) => {
          const x = CHART_MARGIN.left + CHART_WIDTH * ratio
          ...
          return (
            <g key={i}>
              <line
                x1={x + randomX}
                y1={CHART_MARGIN.bottom}
                x2={x + (Math.random() - 0.5) * 2}
                y2={HEIGHT - CHART_MARGIN.top}
                stroke="#444"
                strokeWidth={1.2}
                strokeLinecap="round"
                style={{
              ...
              />
              <text
                x={x + 5 + (Math.random() - 0.5) * 2}
                y={CHART_MARGIN.bottom - 15 + (Math.random() - 0.5) * 2}
                fill="#333"
                fontSize={16}
                fontWeight="bold"
                textAnchor="start"
            ...
            <g key={i}>
              ...
              <text
                ...
                style={{
                  ...
                  transform: `translateY(${-labelProgress * 10}px)`,
                }}
              >
...
export const StarsComposition: React.FC = () => {
  const repoName = "OpenClawDouyinVideo"
  return (
    <>
      {/* 主增长曲线（含开场动画） */}
      <Composition
